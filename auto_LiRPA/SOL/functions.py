...
def torch_geluopenai(x):
    return 0.5 * x * (1.0 + torch_tanh(math.sqrt(2.0 / math.pi) * (x + 0.044715 * x * x * x)))
...
def torch_geluopenai_grad(x):
    grad = 0.5 * torch_tanh(0.0356774 * x ** 3 + 0.797885 * x) + \
           (0.0535161 * x ** 3 + 0.398942 * x) * torch_sech(0.0356774 * x ** 3 + 0.797885 * x) ** 2 + 0.5
    return torch.expand_dims(grad, axis=-1)
...
def torch_tanh_grad(x):
    return (torch_sech(x) ** 2).unsqueeze(-1)
