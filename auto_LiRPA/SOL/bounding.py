...
class OptimalLinearBounder:
    ...
    def _get_G_bound(bound, cell_sides):
        ...
        max_orthogonal_offset = np.sqrt(np.abs(cell_diam ** 2 - cell_sides ** 2)) / 2
    ...
    def _get_most_branched(self, points, cell_diams):
        """
        Gets the point where cells had to be split the most
        :param points:
        :param cell_diams:
        :return:
        """
        points = np.squeeze(points)
        cell_diams = np.squeeze(cell_diams)
        sorted_indices = np.argsort(points)
        points = points[sorted_indices]
        cell_diams = cell_diams[sorted_indices]

        most_branched_idx = np.argwhere(cell_diams == np.min(cell_diams))
        most_branched_idx = most_branched_idx[most_branched_idx.shape[0] // 2, 0]
        most_branched_point = points[most_branched_idx]

        return most_branched_point



    def _bound_one_side(self, bound, upper=True, verbosity=False, get_most_branched=False):
        ...
        while True:
            ...
            if np.all(holds):
                ...
                if not upper:
                    coeffs *= -1
                if verbosity: (f"returned {'upper' if upper else 'lower'} bound in {iter} iterations")
                if get_most_branched:
                    return coeffs, self._get_most_branched(points, cell_diams)
                else:
                    return coeffs, None
            iter += 1
    ...
    def find_optimal_bounds(self, bound, upper=True, lower=True, verbosity=False, get_most_branched=False):
        """
        ...
        assert upper or lower, "Must how to bound this target function"

        if upper and lower:
            return (
                *self._bound_one_side(bound, upper=False, verbosity=verbosity, get_most_branched=get_most_branched),
                *self._bound_one_side(bound, upper=True, verbosity=verbosity, get_most_branched=get_most_branched)
            )
        elif upper:
            return (
                None, None,
